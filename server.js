...
/**
  * PUT /api
  */
 app.put("/api/:id", challengeAuth, (req, res) => {
     const id = req.params.id;
     const updatedDataProperties = req.body;

     HelloModel.findByIdAndUpdate({_id: id}, {$set: updatedDataProperties}, (err, doc)=>{
         if(err){
            res.status(404).send("uh oh! something went wrong on update");
         }
...
/**
  * DELETE /api
  */
 app.delete("/api/:id", challengeAuth, (req, res) => {
     const id = req.params.id;
     HelloModel.findByIdAndRemove({_id: id}, (err, doc)=>{
         if(err){
            res.status(404).send("uh oh! something went wrong on delete");
         }
         HelloModel.find({}, (err, doc)=> {
             // redirect to "GET" all the latest data
             res.redirect("/api")
